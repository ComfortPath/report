...
This illustrates a broader point: file formats should be chosen based on how the data will be used, not only on whether they can store the required information. In this workflow, the environmental data are not only visualized as maps; they are repeatedly queried to annotate many network edges. This is especially relevant for geodata, where the location of each value is part of its meaning and data access is often spatially organized. A format that supports repeated spatial and temporal lookup can therefore provide substantial performance gains.
...
This would have been more difficult with a more static network format such as GML, where the graph representation is embedded in the file structure and additional preprocessing would be needed before the data could be used in array-based routing. This shows that the chosen file formats can either support or constrain flexibility and performance.

=== routing prototype
The routing prototype was the component where all components came together. Compared with other parts of the workflow, the general routing approach was better described in existing research, so the methodological plan could be followed more directly.
...
The development process also showed that the research value of the front-end was more limited than initially expected. For this thesis, the front-end mainly needed to demonstrate that the routing algorithm could be used interactively and that the results could be visualized. Therefore, ease of implementation was more important than developing a fully optimized application. #todo[is this a result actually?]

== Evaluation of the tool
=== Performance of the implemented workflows
TODO: 
- run tests on delftBlue and put them in a table. 
  - todo tests resolution 0.5, 1, 1.5 -> Generate results to put timings in a table Here. DelftBlue + here
  - Eind vandaag die resultaten aanzetten op delft blue zodat ik de tabel in kan vullen!

By integrating into the pedestrian network the size of the .tif (~750MB) becomes 3MB.
=== Route validation
...
#todo[What does one say about maps when they sort of speak for themselves.]
#todo[frequency graph was too difficult to plot in such a way that it gave nwe insights, and dubious comparability since segments have to be compared not distances]
...
The number of newly used edges follows the same pattern as the distance increase @fig:res-diff. Comfortable routes at 15:00 and 18:00 include more edges that are not part of the shortest route, particularly for the longer OD categories. This shows that the algorithm does not only increase route length slightly, but can also select partly different paths through the network when hot edges are present. At 08:00 and 22:00, the number of newly used edges is much lower, which again suggests that the shortest route often already avoids problematic heat exposure at these times.
