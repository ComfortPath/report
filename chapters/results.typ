...
The results show that the preparation and simulation of the @SOLWEIG data are the most computationally demanding parts of the workflow. In particular, @SOLWEIG execution time decreases substantially when the spatial resolution is reduced. This is expected, because a coarser resolution reduces the number of raster cells that need to be processed during the simulation. 

The test on a different area was included to assess whether the workflow could be applied outside the original study area without requiring changes. No implementation issues were encountered: the new bounding box could be inserted into the same data preparation, @SOLWEIG execution, and network annotation steps. The calculation times were slightly lower than for the original study area, which is caused by the smaller spatial extent of the second area.
...
The paths shown in @fig:utci-on-route indicate which raster cells were sampled along each route. The cells sampled along the thermally conscious route generally have lower @UTCI values than the corresponding alternatives along the shortest route. A closer inspection of the original @UTCI map around the route in @fig:route-on-utci #footnote[The figure legend scale is capped at the maximum value sampled along the route to increase contrast for route-level analysis. This was necessary because some disproportionately high @UTCI values outside the route area skew the colour scale.] shows that the thermally conscious route mainly follows cooler areas associated with vegetation. These areas are visible on the @UTCI map as rounded cool spots, which correspond to tree locations.

The increased presence of vegetation along the thermally conscious route is also visible in the satellite imagery shown in @fig:sat-roads. At the first deviation between the two routes for OD pair 229, the thermally conscious route follows `Roerdomplaan` and `Lepelaarsingel`, rather than the shortest alternative which sends you via `Dorpsweg` and `Wielewaalstraat`. The satellite imagery indicates that `Wielewaalstraat` contains relatively little vegetation, which is consistent with the higher UTCI values assigned to this segment. In contrast, `Roerdomplaan` is characterized by a greater presence of vegetation and is therefore selected by the routing algorithm as the thermally preferable alternative.
#figure(
...
Users can configure the relative importance of the routing weights. For these tests, the default settings were used, with the @UTCI weight set to 1. Under this setting, the heat penalty is determined by the multiplier scheme described in @chap:methodology and @tab:heat_multipliers, while consecutive exposure to hot edges is additionally penalized.
...
The average distance increase in the higher distance categories is about 70m which is the same as perceived distance increase caused by one extra degree in the UTCI @basu_hot_2024. The average distance increase for more comfortable routes is around 4\% across categories which is a little lower than previous work @rusig_reducing_2017 @ma_active_2025. Indicating that there is potentially more room for deviation from the shortest path if additional variables are added to the algorithm.
...
The reduction in UTCI at 12:00 is lowest of the 'hot times' measurements. This could be caused by the high position of the sun, which reduces the extent to which urban geometries such as buildings and vegetation cast shade. A similar effect was observed by Wen et al. (2025) at this time @wen_walking_smart_2025. In addition, the limited number of edges classified into the highest UTCI categories, as shown by the lower average UTCI as compared to 15:00 and 18:00, may reduce the influence of the heat-penalty factors. As a result, the routing algorithm has fewer strongly penalized edges to avoid and is therefore less likely to select a substantially different route. The reduction in UTCI is generally larger for the longer distance categories, where more alternative edges are available.

The reductions in @UTCI are slightly lower than those reported in previous work @rusig_reducing_2017 @ma_active_2025. This may partly be explained by a relatively lower importance assigned to heat avoidance in the routing weights, which also limits the additional distance the algorithm is willing to introduce. In the study by Rußig and Bruns (2017), the average route length was approximately 2 km; since @UTCI reductions become more pronounced as route length increases, this may partly account for the larger reductions reported in their results. The comparison with Ma et al. (2025) should also be interpreted cautiously, as their results were obtained in the different climatic context of Hong Kong, where thermal conditions may differ substantially.
...
#figure(
  ...
  gutter: 1em,
  align: alignment.horizon,
    image("../figs/results/structured/distance_difference_vs_utci_difference_category_max_distance.png", width: 90%),
    image("../figs/results/structured/distance_difference_vs_utci_difference_category_medium_long.png", width: 90%),
    image("../figs/results/structured/distance_difference_vs_utci_difference_category_medium.png", width: 90%),
    image("../figs/results/structured/distance_difference_vs_utci_difference_category_short_mid.png", width: 90%),
    image("../figs/results/structured/distance_difference_vs_utci_difference_category_short.png", width: 90%)
  ),
  caption: "Distance (x-axis), UTCI difference (y-axis), between the shortest and most comfortable route"
)<fig:res-ut-di>
